...
func (g guardEntry) String() string {
	id := ""
	if g.id != 0 {
	...
	return g.Format("20060102 04") + action + id
}

type guardLog []guardEntry

func (g guardLog) Len() int {
	return len(g)
}

func (g guardLog) Less(i int, j int) bool {
	return g[i].Time.Before(g[j].Time)
}

func (g guardLog) Swap(i, j int) {
	g[i], g[j] = g[j], g[i]
}
...
func day4(input string) (int, int, error) {
	...
	for _, line := range lines {
		...
		det := line[19:]
		switch {
		case strings.HasPrefix(det, "Guard"):
			var id int
			fmt.Sscanf(det, "Guard #%d begins shift", &id)
			gl = append(gl, guardEntry{Time: t, action: begin, id: id})
		case det == "falls asleep":
			gl = append(gl, guardEntry{Time: t, action: sleep})
		case det == "wakes up":
	...
	sort.Sort(gl)
	var curGuard, curSleepMM int
	days := make(map[string]int) // to guard on duty that day
	dayminutes := make(map[string]map[int]int)
	guardminutes := make(map[int]int)
	maxMM := 0
	guardmax := -1
	for i := range gl {
		log.Println(gl[i])
		daystr := gl[i].Format("20060102")
		// todo, if it is a new day, mark the guard as woke at end of midnight hour?
		switch gl[i].action {
		case begin:
			curGuard = gl[i].id
		case sleep:
			days[daystr] = curGuard
			curSleepMM = gl[i].Minute()
		case wake:
			days[daystr] = curGuard
			guardminutes[curGuard] += gl[i].Minute() - curSleepMM
			if guardminutes[curGuard] > maxMM {
				maxMM = guardminutes[curGuard]
				guardmax = curGuard
				log.Printf("guard #%d now has the most minutes with %d", guardmax, maxMM)
			}
			for j := curSleepMM; j < gl[i].Minute(); j++ {
				if _, ok := dayminutes[daystr]; !ok {
					dayminutes[daystr] = make(map[int]int)
				}
...
var day4Input = `[1518-08-29 00:24] falls asleep
